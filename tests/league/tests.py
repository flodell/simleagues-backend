...
class LeagueAPITestCase(APITestCase):
    ...
    def test_update_league_as_non_staff(self):
        ...
        LeagueMembership.objects.create(
            league=self.league1, user=self.user3, role=LeagueMemberRole.MEMBER
        )
    ...
    def test_hard_delete_without_confirmation(self):
        ...
        self.client.force_authenticate(user=self.user1)
        url = (
            reverse("league-detail", kwargs={"pk": self.league1.pk})
            + "?hard_delete=true"
        )
        response = self.client.delete(url)
    ...
    def test_hard_delete_with_championships(self):
        ...
        Championship.objects.create(
            league=self.league1, name="Test Championship", start_date="2025-01-01"
        )
    ...
    def test_restore_as_non_admin_fails(self):
        ...
        LeagueMembership.objects.create(
            league=self.inactive_league, user=self.user2, role=LeagueMemberRole.MEMBER
        )
    ...
    def test_join_public_league(self):
        ...
        self.assertTrue(
            LeagueMembership.objects.filter(
                league=self.league1, user=self.user3
            ).exists()
        )
    ...
    def test_admin_cannot_demote_another_admin(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn(
            "cannot change role of another admin", response.data["detail"].lower()
        )
    ...
    def test_admin_can_promote_member_to_admin(self):
        ...
        # Add user2 as member
        LeagueMembership.objects.create(
            league=self.league1, user=self.user2, role=LeagueMemberRole.MEMBER
        )
    ...
    def test_admin_can_demote_moderator(self):
        ...
        LeagueMembership.objects.create(
            league=self.league1, user=self.user2, role=LeagueMemberRole.MODERATOR
        )
