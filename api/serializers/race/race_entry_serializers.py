...
class RaceEntryCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if championship_entry:
            ...
            if data.get("car") or data.get("racing_number") or data.get("team") or data.get("user"):
                errors["championship_entry"] = "Cannot specify car, racing_number, team or user when championship_entry is set."
        else:
            # Standalone context
            ...
            if not data.get("racing_number"):
                errors["racing_number"] = "Racing number is required for standalone entries."

            # Check racing number uniqueness
            if data.get("racing_number") and RaceEntry.objects.filter(
